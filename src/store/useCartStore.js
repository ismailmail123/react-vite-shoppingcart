...
const useCartStore = create(
    ...
    (set, get) => ({
        ...
        addCartItems: (item) => {
            ...
            const product = get().products.find((product) => product.id === item.id);

            if (product && product.stock > 0) { // Periksa ketersediaan stok
                if (itemExists) {
                    if (typeof itemExists.quantity == "number") {
                        itemExists.quantity++;
                    }
        ...
        increaseQuantity: (productId) => {
            ...
            if (itemExists && product) {
                if (typeof itemExists.quantity === "number") {
                    // Periksa apakah stok mencukupi sebelum menambah jumlah
                    if (product.stock > 0) {
                        itemExists.quantity++;
